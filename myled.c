 ...
 */
 ...
 if (!request_mem_region(res->start, remap_size, pdev->name)) {//请求内存区域
 dev_err(&pdev->dev, "Cannot request IO\n");
 return -ENXIO;//没有设备或地址
 }

 base_addr = ioremap(res->start, remap_size);//映射到虚拟地址
 if (base_addr == NULL) {
...
ed_proc_entry = proc_create(DRIVER_NAME, 0, NULL,//创建一个proc虚拟文件
